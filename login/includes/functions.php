<?php
	...
	//require_once 'database_access/access_temp.php';
    ...
    if ($stmt = $mysqli->prepare("SELECT id, email, password FROM helper WHERE email = ?
        LIMIT 1")) {
		...
		//Wenn User nicht in der Tabelle Members vorhanden hiernach
		if($stmt->num_rows == 0){

			$stmt = $mysqli->prepare("SELECT id, email, password FROM members WHERE email = ? LIMIT 1");
	        $stmt->bind_param('s', $email);  // Bind "$email" to parameter.
	        $stmt->execute();    // Führe die vorbereitete Anfrage aus.
	        $stmt->store_result();
